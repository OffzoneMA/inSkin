...
function Changepassword({ navigation }) {
  ...
  const handleComparePassword = async (currentPassword) => {
    try {
      const result = await authApi.comparePassword(user._id, currentPassword);
  ...
  const saveHandler = async ({
    ...
    const saveHandler = async ({
      ...
      // Comparer le mot de passe avant de continuer
      const result = await handleComparePassword(currentPassword);
  
      
  
      if (!result.ok) {
        console.log("mot de passe erreur");
        return;
      }
        ...
        <View >
          ...
          >
            {({
              ...
              <>
                <ScrollView>
                <Text style={style.label}>Your password</Text>
               ...
               <View style={{flexDirection: "row", justifyContent: "space-between"}}>
                    <Button loading={updateUserInfoApi.loading} title="OK" onPress={handleSubmit} style={{
    marginTop: 20,
    opacity:  values.currentPassword ? 1 : 0.5,
